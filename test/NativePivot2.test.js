...
const Pivot = contract.fromArtifact('NativePivot2')
...
describe('NativePivot V. 2.0', function () {
  ...
  beforeEach(async function () {
    ...
    this.pivot = await Pivot.new(this.token.address, min_expire, min_lock, min_ask, min_strike, fee, { from: guy })
  ...
  describe('Initialization Revert', () => {
    ...
    it("zero token", async function () {
      await expectRevert(
        Pivot.new(ZERO_ADDRESS, min_expire, min_lock, min_ask, min_strike, fee, { from: guy }),
        "Invalid token"
      )
    })
    ...
    it("zero min_lock", async function () {
      await expectRevert(
        Pivot.new(this.token.address, min_expire, zero, min_ask, min_strike, fee, { from: guy }),
        "Invalid min_lock"
      )
    })
    ...
    it("zero min_expire", async function () {
      await expectRevert(
        Pivot.new(this.token.address, zero, min_lock, min_ask, min_strike, fee, { from: guy }),
        "Invalid min_expire"
      )
    })
    ...
    it("zero min_ask", async function () {
      await expectRevert(
        Pivot.new(this.token.address, min_expire, min_lock, zero, min_strike, fee, { from: guy }),
        "Invalid min_ask"
      )
    })
    ...
    it("zero min_ask", async function () {
      await expectRevert(
        Pivot.new(this.token.address, min_expire, min_lock, min_ask, zero, fee, { from: guy }),
        "Invalid min_strike"
      )
    })
  })
  ...
  describe('Join', () => {
    it("should join", async function() {
      ...
      await this.pivot.join(id1, expire.add(timestamp), ask, strike, {from: guy, value: lock})
    ...
    describe('Revert', async function() {
      ...
      it("should join only once per id", async function() {
        await this.pivot.join(id1, expire.add(timestamp), ask, strike, {from: guy, value: lock})
        await expectRevert(this.pivot.join(id1, expire.add(timestamp), ask, strike, {from: guy, value: lock}), 
            "Taken"
        )
      })
      ...
      it("shouldnt join with value 0", async function() {
        await expectRevert(
            this.pivot.join(id1, expire.add(timestamp), ask, strike, {from: guy, value: 0}), 
      ...
      it("shouldnt join with value less than min_lock", async function() {
        await expectRevert(
            this.pivot.join(id1, expire.add(timestamp), ask, strike, {from: guy, value: 1}), 
            "Value is less than min_lock"
        )
      })
      ...
      it("shouldnt join with 0 ask", async function() {
        await expectRevert(
            this.pivot.join(id1, expire.add(timestamp), zero, strike, {from: guy, value: lock}), 
      ...
      it("shouldnt join with 0 strike", async function() {
        await expectRevert(
            this.pivot.join(id1, expire.add(timestamp), ask, zero, {from: guy, value: lock}), 
            "Strike less than min_strike"
        )
      })

      it("shouldnt join with expire less than timestamp", async function() {
        await expectRevert(this.pivot.join(id1, zero, ask, strike, {from: guy, value: lock}), 
  ...
  describe('Claim', () => {
    ...
    describe('Revert', async function() {
      ...
      it("origin shouldnt claim", async function() {
        timestamp = await time.latest();
        await this.pivot.join(id2, expire.add(timestamp), ask, strike, {from: guy, value: lock})
  ...
  describe('Back', () => {
    ...
    describe('Revert', async function() {
      ...
      it("shouldnt back if not expired", async function() {
        await this.pivot.join(id2, (await time.latest()).add(new BN(40000)), ask, strike, {from: guy, value: lock})
