...
// V3
contract NativePivot2 is Ownable, Eventful {
    ...
    constructor(address _dai, uint256 _min_expire, uint256 _min_lock, uint256 _min_ask, uint256 _min_strike, uint256 _fee) public {
    ...
    function join(bytes32 id, uint256 expire, uint256 ask, uint256 strike) public payable returns(bool) {
        ...
        opts[id] = Option(msg.sender, msg.value, expire, strike, ask);
        composite[id] = msg.value;
    ...
    function back(bytes32 id) public returns(bool) {
        ...
        opts[id].origin.transfer(composite[id]);
        composite[id] = 0;
